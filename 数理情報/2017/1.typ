...
$x = 1$が$g(x) = 0$の唯一の解で、$f(x) = 0$である。\
  ...
  であり$g'$は単調増加する。
  $
    g'(x) = a^x (log a) - 1 &= 0 <=> x = - (log (log a))/(log a)
  $
  より、関数$g$は$tilde(x) = - (log (log a))/(log a)$で最小値$g(tilde(x)) = (1 + log (log a))/(log a)$をとる。\
  $lim_(x -> ± infinity) g(x) = infinity$に注意すると、$g(x) = 0$が実数解を持つための必要十分条件は、
  $
    g(tilde(x)) = (1 + log (log a))/(log a) <= 0 <=> a <= e^(1/e)
  $
    $a = e^(1/e)$のとき、$x = tilde(x)$が$g(x) = 0$の唯一の解だが$f(tilde(x)) = 1$より不適。\
    $a < e^(1/e)$のとき、最小値$g(tilde(x))$は$0$より小さいので$g(x) = 0$は２つの実数解を持ち、小さい順に$x_1 ,x_2$とおくと、$0 < x_1 < tilde(x) < x_2$である。よって、$0 < f(x_1) < f(tilde(x)) = 1 < f(x_2)$なので、$a > 1$の場合に安定な不動点が存在する必要十分条件として$1 < a <e^(1/e)$が得られる。\
  ...
  $
    ...
    e^(x/e) = x &<=> x = e 
  $
  から、対応する$macron(x)$の範囲は$1 < macron(x) < e$である。\
  \
  - *$a < 1$の場合*\
  $g'(x) = a^x (log a) - 1 < 0$より$g$は単調増加で$g(0) = 1 > 0, lim_(x ->  infinity) g(x) = - infinity$なので、$g(x) = 0$は唯一の実数解$x = macron(x) >0$を持つ。\
...
より$macron(x)$は$a$に関して単調増加する。
  ...
  まとめると、安定な不動点が存在するための$a$に関する必要十分条件は$e^(- 1/e) < a < e^(1/e)$であり、このときの$macron(x)$の範囲は$e^(-1) < macron(x) < e$である。