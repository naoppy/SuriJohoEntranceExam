...
== (3)
$L$の非零固有値$alpha$と対応する固有ベクトル$u = mat(u_1,...,u_n)^⊤$を任意にとる。
  $
    ...
    &= 0
  $
$alpha != 0$なので$sum_(i=1)^n u_i = mat(1, ... ,1) u = 0$である。
== (4)
$L$は対称行列なので固有値は実数である。\
任意の$x in RR^n$に対して、
  $
...
$G$は連結なので$x L = 0$の解は$x = t (1,...,1)$（$t in RR$）のみであることから、$L$の$0$固有ベクトルは$t (1,...,1)$（$t in RR without {0}$）。$L$の固有値は非負なので、$x(t)$の$0$固有ベクトルの成分以外の成分は$t -> infinity$で$0$に収束する、すなわち、
  $
