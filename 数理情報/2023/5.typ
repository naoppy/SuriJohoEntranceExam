...
$n$が偶数であれば,$ 2^n + 2^(n - 2) + dots + 2^0 = (4^(n/2+1) - 1 ) / (4 - 1) $
$n$が奇数であれば, $ 2^n + 2^(n-2) + dots + 2^1 = 2 (4^((n+1)/2) - 1) / (4 - 1) $
== (2)
...
$$
観察すると$11$を$10-1$に変更するとよさそうなことがわかる.
...
これをまとめると$ T_(i) = S_i + sum_(j = 1)^(i-2) 2T_(j) $和の漸化式の鉄則に従って差分をとる.
$ T_i - T_(i - 1) = 2T_(i - 2) +S_i - S_(i - 1) $
$ T_i - T_(i - 1) - 2T_(i-2) = S_i - S_(i-1) = (2^(i - 1) - 2 (-1)^i) / (3) $
この漸化式の特性方程式の根は$2,-1$より,一般解$ T_i = alpha 2^i + beta (-1)^i $微分方程式の類推で, 非斉次問題に対する解を$ (alpha i + beta)2^i + (gamma i + delta)(-1)^i $と仮定する.
元の式に代入して係数間の関係を調べると
