...
まずは, $u>0$で$ 1 / L dif/(dif u)L = - 1/(sqrt(2u)) $
$ log L = - sqrt(2) u^(1/2) + C $
$ L = A exp(-sqrt(2u)) $
初期条件$u = 0$で1より$ L = exp(-sqrt(2u)) $
次に複素数に拡張します.

そもそも求めたいものは
$ integral_0^infinity e^(i u y) 1/sqrt(2pi) y^(-3/2)e^(-1/(2y))dif y $
です. $u$を複素数に拡張して, 複素平面でこの関数を計算し, 実軸に対応する部分が求める答えになります.この関数は虚軸正部分$u = i u^prime, u^prime>0$で$L = exp(-sqrt(2u^prime))$に一致します. この関数は, 分枝を除いて正則なので, 複素関数の一致の定理を用いて拡張することができます.

$L^prime (z)= exp(-sqrt(2e^(-pi/2) z))$として分枝は虚軸負の方向に取っておき, $L^prime (i u) = exp(-sqrt(2u))$となるようにします.

このとき,$z = R e^(i phi), -pi/2<=phi <3/2pi$とします. この時実際に$phi = pi/2$であると, $L = exp(-sqrt(2R))$となります.

$u$が実数であることからもう少し表式を綺麗にしていきます.$u = 0$の時はもちろん$1$です.$u>0$のとき,極座標表示すると$u = u e^(i 0)$となります. これを代入して計算すると,$ L(u) = exp(-sqrt(2u)sqrt(e^(-i pi/2))) = exp(-sqrt(2u)e^(-i pi/4)) = exp(-(sqrt(2u)) (1- i) /sqrt(2)) = exp((-1 + i)sqrt(u)) $
