...
$
これは, 行列が対称になるように変形した. さて問題の条件から $C A + A^T C$ は負定値である. ここで$W X^* = C$となるように選べば, $(dif H_w (z(t))) / (dif t)$ の第一項は負の値になる. 第二項が $0$ の $epsilon$ 近傍では第一項に比べて無視できることから, $W X^* = C$ が一つの解となる.明示的に書けば,
$ w_i = c_i /x_i $
