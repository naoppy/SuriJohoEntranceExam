...
// という問題ですが, 
定義に従って計算していけば何とかなります. 問題を示すには対数の中身が $1$ より大きければよいことに注意します.
$
...
== (2)
// $f(A,C)<=f(A,B) + f(B, C)$を示せという問題です.
いわゆる三角不等式の成立を問う問題です. 同じく同値変形したり緩い条件に変えたりして示します. 緩い評価は総和の各項が不等式を満たすなら, 総和をとっても不等式を満たすという評価を用います.(もちろん一般には総和の各項が不等式を満たしていなくても和を取ったら成り立つことがあります.) まず, $log$ の性質から
$ 
  &f(A,C) <= f(A,B) + f(B, C) \
  &<=> 1 / (m) sum_(i = 1)^(m) 1 / (1 / (l) sum_(k = 1)^ (l)e ^ norm(a_i - c_k)) <= 1 / (m) sum_(i = 1)^(m) 1 / (1 / (n) sum_(j = 1)^ (n)e ^ norm(a_i - b_j))1 / (n) sum_(j = 1)^(n) 1 / (1 / (l) sum_(k = 1)^ (l)e ^ norm(b_j - c_k))
$
