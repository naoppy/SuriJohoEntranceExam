...
#proof[
  ...
  よって、 $g_(k-1) (x)$ は $f_(k-1) (x)$ と同じ単調減少する区間を持ち、単調増加する区間は全て定数関数 $f_(k-1) (p_(k-1))$ に置き換えられている。つまり、$g_(k-1)$ は最大 $k$ 個の区間を持つ、区分二次な広義凸関数である。
...
さて、これで基本的な説明は終わった。
次の問題は、この区間とそこでの関数をどのように表現するかである。区間は被りがないので、各区間の端点とその区間での二次関数の係数 $a x^2 + b x + c$ の $(a, b, c)$ を保存すればよい。二次関数が足されるときはそのまま係数同士を足せばよいし、最小な点 $p_k$ を求めるときは、各区間の二次関数の最小点 $x = - b / (2a)$ を求めて、それが区間内に入っていればそれが最小点である。入っていなければ、隣の区間を順番に探索していけばよい。
...
さらに、区間の係数を持つのではなく、区間の区分点において、左右の係数の差を持つようにすればよい。こうすると区間に二次関数を足すとき、最も右の区間の係数を増やすだけでよくなる。これは slope trick と呼ばれるテクニックで、競技プログラミングでよく使われる。しかも、
このような高速化をすると全体で $O(n)$ になる。とはいえこれらのテクニックを使わず愚直に実装しても $O(n^2)$ で多項式時間なので、問題に解答する分には十分であるが、一応紹介しておいた。
